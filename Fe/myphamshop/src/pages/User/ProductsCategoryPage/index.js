...
const ProductCategoryPage = () => {
    ...
    useEffect(() => {
        ...
        axios.get(`https://localhost:7007/api/Category/category/${categoryId}`)
            .then((response) => {
                // Kiểm tra dữ liệu trả về có phải đối tượng với mảng products hay không
                if (response.data && Array.isArray(response.data.products)) {
                    setProducts(response.data.products); // Cập nhật danh sách sản phẩm
                } else {
                    setProducts([]); // Nếu không có mảng products, gán mảng rỗng
                    console.error("Dữ liệu trả về không chứa mảng sản phẩm");
                }
